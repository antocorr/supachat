...
const LOGGING_DEFAULTS = Object.freeze({
    notFound: 'error',
    permission: 'error',
    forbidden: 'error',
    validationFailed: 'error',
});
const LOGGING_MODES = new Set([false, 'log', 'info', 'warn', 'error', 'throw']);
const LOGGING_FN_KEYS = new Set(['logFunction', 'infoFunction', 'warnFunction', 'errorFunction']);
...
const RECEIVER = Symbol('rpcable.receiver');
...
export function getReceiver(proxy) {
    return getInstance(proxy)?.[RECEIVER] ?? null;
}
...
export class RpcAble {
    ...
    flush() {
        if (this.transport !== 'collector') {
            console.warn('[RpcAble] flush() is only available on collector transport');
            return [];
        }
    ...
    async _flush() {
        ...
        try {
            const res = await this.fetchImpl.call(globalThis, this.endpoint, {
                method: 'POST',
...
export class RpcAbleReceiver {
    ...
    updateSettings(key, data) {
        if (key === 'logging') {
            for (const [k, v] of Object.entries(data)) {
                if (k in LOGGING_DEFAULTS) {
                    this._logging[k] = LOGGING_MODES.has(v) ? v : LOGGING_DEFAULTS[k];
                } else if (LOGGING_FN_KEYS.has(k)) {
                    this._logging[k] = typeof v === 'function' ? v : undefined;
                }
            }
    ...
    resetSettings(key, data = null) {
        if (key === 'logging') {
            this._logging = { ...LOGGING_DEFAULTS };
    ...
    _emitReceiverLog(kind, message) {
        const mode = this._logging[kind];
        if (mode === false) return;
        if (mode === 'throw') throw new Error(message);
        if (mode === 'log') { (this._logging.logFunction ?? console.log)(message); return; }
        if (mode === 'info') { (this._logging.infoFunction ?? console.info)(message); return; }
        if (mode === 'warn') { (this._logging.warnFunction ?? console.warn)(message); return; }
        (this._logging.errorFunction ?? console.error)(message);
    }
    ...
    async _invokeMethod(role, path, args) {
        ...
        if (typeof current === 'function') {
            const callArgs = role !== null ? [...args, role] : args;
    ...
    _sendResponse(payload) {
        ...
        try {
            ...
            } else if (typeof client[RESPONSE_PATH] === 'function') {
...
export function createBiDirectionalSession(conf = {}) {
    const transport = conf.transport ?? 'socketio';
    const channel = conf.channel ?? '-userSession';
    const socket = conf.socket ?? null;
    const getRole = typeof conf.role === 'function' ? conf.role : () => conf.role ?? null;

    if (!socket) throw new Error('[RpcAble] createBiDirectionalSession requires a socket');
    ...
    const userSession = new RpcAble({ transport, socket, channel, target: conf.target });
    getInstance(userSession)[RECEIVER] = receiver;

    if (transport === 'socketio') {
        socket.on(channel, (batch) => {
            const role = getRole();
            receiver.dispatch(batch, role !== null ? { role } : null);
        });
    } else if (transport === 'websocket' && typeof socket.addEventListener === 'function') {
